...
/* ----------------------------------------------------------------------
 *  无操作分级自动关闭 (秒, 只按键算操作)
 *    30min 无操作 -> 关闭 LDM 测距 (SW3)
 *    2h   无操作 -> 关闭 laser 十字线 (SW4/T7)
 *  之后保持待机, 直到低电/手动长按/BUCK 失效才整机关机。
 * ---------------------------------------------------------------------- */
#define LDM_AUTOOFF_SEC      (1800U)  /* 30min 关 LDM */
#define LASER_AUTOOFF_SEC    (7200U)  /* 2h   关 laser */
 ...
 * ---------------------------------------------------------------------- */
#define IMU_DIR_HYST_TH       (4000)                                                 /* 迟滞死区阈值 (LSB) */
...
/* ----------------------------------------------------------------------
 *  电池电量阈值 (ADC 原始值, 实测标定)。
 *  阈值随负载动态调整: 基准为「都关」(SW3 LDM 关 + SW4 T7 关),
 *  负载越大阈值越低 (大负载下电池压降更大, 需更低截止电压充分利用电池):
 *    SW4 (T7) 开  -> 下调 10
 *    SW3 (LDM) 开 -> 下调 20
 *  各状态取值 (原始值):
 *    关机 OFF:  都关 1135 / 只SW4 1125 / 只SW3 1115 / 都开 1105
 *    3->2 格:   都关 1340 / 只SW4 1330 / 只SW3 1320 / 都开 1310
 *    2->1 格:   都关 1225 / 只SW4 1215 / 只SW3 1205 / 都开 1195
 * ---------------------------------------------------------------------- */
#define ADC_BATT_OFF_BASE      (1135U)                                                /* 关机阈值基准 (都关) */
